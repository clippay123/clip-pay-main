...
export const getSpecificCreatorCampaigns = async () => {
  ...
  try {
    // Fetch campaign IDs where the user has made a submission
    ...
    // Extract campaign IDs from submissions
    const campaignIds = submissions.map((sub) => sub.campaign_id)

    if (campaignIds.length === 0) {
      return [] // No submissions found for this user
    }

    // Fetch only campaigns where the user has made a submission
    const { data: campaigns, error } = await supabase
      ...
      .in("id", campaignIds) // Fetch only campaigns in the submission list
      ...
      (campaign: any) => {
        ...
        const transformedSubmission = userSubmission
          ? {
              ...
              previous_views: userSubmission.previous_views,
    ...
    console.error("Unexpected error in getSpecificCreatorCampaigns:", error)
    throw error
  }
}
